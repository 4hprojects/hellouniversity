...
function createAuthWebRoutes({
  ...
  router.get('/login', (req, res) => {
    ...
    if (req.session && req.session.userId) {
      return res.redirect(
        returnTo || getDashboardPathForRole(req.session.role),
      );
    }
  ...
  router.get('/approval-pending', async (req, res) => {
    if (
      req.session?.userId &&
      req.session?.role &&
      req.session.role !== 'teacher_pending'
    ) {
      return res.redirect(getDashboardPathForRole(req.session.role));
    }
    ...
    try {
      ...
      if (!passwordMatch) {
        ...
        if (invalidAttempts >= 3) {
          updateFields.accountLockedUntil = new Date(
            Date.now() + 30 * 60 * 1000,
          );
          ...
          return res.status(403).json({
            success: false,
            message:
              'Account is locked due to multiple failed login attempts. Use password reset if needed.',
          });
        }
      ...
      if (error?.code === 'ECONNRESET') {
        return res.status(503).json({
          success: false,
          message:
            'Temporary connection issue while logging in. Please try again.',
        });
      }
      return res
        .status(500)
  ...
  router.get('/api/csrf-token', isAuthenticated, (req, res) => {
    ...
    if (!token) {
      return res
        .status(500)
        .json({ success: false, message: 'Unable to issue CSRF token.' });
    }
