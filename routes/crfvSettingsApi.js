...
router.get(
  ...
  async (_req, res) => {
    try {
      ...
      console.error(
        'Error in GET /api/crfv/settings/attendance-defaults:',
        error,
      );
...
router.put(
  ...
  async (req, res) => {
    const { schedule, errors } = validateAttendanceSchedule(
      req.body?.attendance_schedule,
    );
    if (errors.length > 0) {
    ...
    try {
      ...
      const updatedSchedule = await updateAttendanceDefaults(
        schedule,
      ...
      console.error(
        'Error in PUT /api/crfv/settings/attendance-defaults:',
        error,
      );
      return res.status(500).json({
        success: false,
